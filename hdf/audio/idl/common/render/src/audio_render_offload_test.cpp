/*
 * Copyright (c) Huawei Device Co., Ltd. 2024.
...
void AudioUtRenderOffloadTest::InitRenderAttrs(struct AudioSampleAttributes &attrs)
{
...
void AudioUtRenderOffloadTest::FreeAdapterElements(struct AudioAdapterDescriptor *dataBlock, bool freeSelf)
{
...
void AudioUtRenderOffloadTest::ReleaseAllAdapterDescs(struct AudioAdapterDescriptor *descs, uint32_t descsLen)
{
...
void AudioUtRenderOffloadTest::SetUp()
{
...
void AudioUtRenderOffloadTest::TearDown()
{
...
/**
 * @tc.number: SUB_Driver_Audio_RenderHdi_9600
 * @tc.name: RenderSetBufferSizeIsInValid001
 * @tc.desc: Verify the SetBufferSize function when the input parameter is invalid
 */
HWTEST_F(AudioUtRenderOffloadTest, RenderSetBufferSizeIsInValid001, TestSize.Level1)
{
    uint32_t size = AUDIO_OFFLOAD_BUFFER_SIZE;
    int32_t ret = render_->SetBufferSize(nullptr, size);
    ASSERT_TRUE(ret == HDF_ERR_INVALID_OBJECT);
}

/**
 * @tc.number: SUB_Driver_Audio_RenderHdi_9700
 * @tc.name: RenderSetBufferSizeIsValid001
 * @tc.desc: Verify the size of SetBufferSize function when distribute offload flow
 */
HWTEST_F(AudioUtRenderOffloadTest, RenderSetBufferSizeIsValid001, TestSize.Level1)
{
    uint32_t size = AUDIO_OFFLOAD_BUFFER_SIZE;
