...
void SaveFile(const char *fileName, uint8_t *data, int size)
{
    if (fileName != nullptr && data != nullptr) {
        ...
        DISPLAY_TEST_LOGE("SaveFile hasWriten %{public}d", hasWriten);
        close(fileFd);
    } else {
...
{
    DISPLAY_TEST_LOGE("x %{public}d, y %{public}d w %{public}d h %{public}d color %x ", rect.x, rect.y, rect.w, rect.h,
        color);
...
std::vector<IRect> SplitBuffer(const BufferHandle &handle, std::vector<uint32_t> &colors)
{
    ...
    DISPLAY_TEST_LOGE("ROW_NUM %{public}u, COL_NUM %{public}u CELL_WIDTH %{public}u CELL_HEIGHT %{public}u",
