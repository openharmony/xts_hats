...
{
    DISPLAY_TEST_LOGE("outputId %{public}u connected %{public}d", outputId, connected);
...
std::shared_ptr<HdiTestDisplay> HdiTestDevice::GetDisplayFromId(uint32_t id)
{
    ...
    DISPLAY_TEST_CHK_RETURN((iter == displays_.end()), nullptr, DISPLAY_TEST_LOGE("can not find the display %{public}u",
        id));
    return displays_[id];
}
...
std::shared_ptr<HdiTestDisplay> HdiTestDevice::FindDisplayOrCreate(uint32_t id)
{
    ...
    if (display == nullptr) {
        DISPLAY_TEST_LOGE("the display not find will creat a display");
    }
