...
int32_t HdiTestDisplay::Init()
{
    ...
    DISPLAY_TEST_LOGE("the capablility name %s type : %{public}d phyWidth : %{public}d phyHeight : %{public}d",
    ...
    DISPLAY_TEST_LOGE("the modes size() %{public}zu", modes_.size());
...
int32_t HdiTestDisplay::GetModeInfoFromId(int32_t id, DisplayModeInfo& modeInfo) const
{
    ...
    if (iter != std::end(modes_)) {
        modeInfo = *iter;
        DISPLAY_TEST_LOGE("the mode width: %{public}d height : %{public}d freshRate : %{public}u id: %{public}d",
...
std::shared_ptr<HdiTestLayer> HdiTestDisplay::CreateHdiTestLayer(LayerInfo& info)
{
    ...
    DISPLAY_TEST_LOGE("CreateLayer layerId %{public}u", layerId);
...
int32_t HdiTestDisplay::RefreshLayersCompType()
{
    ...
    DISPLAY_TEST_LOGE("the change numbers %{public}zu, layers size %{public}zu", layers.size(), layers.size());
    for (uint32_t i = 0; i < layers.size(); i++) {
        DISPLAY_TEST_LOGE(" the layer id %{public}u ", layers[i]);
...
int32_t HdiTestDisplay::GetLayersReleaseFence()
{
    ...
    DISPLAY_TEST_LOGE("the release fence numbers %{public}zu, layers size %{public}zu", layers.size(), layers.size());
    for (uint32_t i = 0; i < layers.size(); i++) {
        DISPLAY_TEST_LOGE(" the layer id %{public}u, fence: 0x%x", layers[i], fences[i]);
        std::shared_ptr<HdiTestLayer> layer = GetLayerFromId(layers[i]);
...
int32_t HdiTestDisplay::PrepareDisplayLayers()
{
    int ret;
    needFlushFb_ = false;
    DISPLAY_TEST_LOGE("id : %{public}u  layer size %{public}zu", id_, layerMaps_.size());
...
int32_t HdiTestDisplay::Commit()
{
    ...
    if (needFlushFb_) {
        DISPLAY_TEST_LOGE("commit out client buffer fence: %{public}d", fenceFd);
...
void HdiTestDisplay::Clear()
{
    ...
    layerMaps_.clear();
    DISPLAY_TEST_LOGE("layerMaps_ size %{public}zu", layerMaps_.size());
}
} // namespace TEST
} // namespace Display
} // namespace HDI
