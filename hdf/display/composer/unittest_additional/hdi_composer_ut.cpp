...
{
    auto ret = g_composerDevice->SetDisplayPowerStatus(g_displayIds[0],
        Composer::V1_0::DispPowerStatus::POWER_STATUS_OFF);
    EXPECT_EQ(DISPLAY_SUCCESS, ret);
}
...
{
    auto ret = g_composerDevice->DestroyLayer(0, layerId002);
    if (layerId002 < 0) {
        EXPECT_EQ(DISPLAY_FAILURE, ret);
    } else {
        EXPECT_EQ(DISPLAY_SUCCESS, ret);
...
{
    auto ret = g_composerDevice->DestroyLayer(g_displayIds[0], layerId007);
    if (layerId007 < 0) {
        EXPECT_EQ(DISPLAY_FAILURE, ret);
    } else {
        EXPECT_EQ(DISPLAY_SUCCESS, ret);
