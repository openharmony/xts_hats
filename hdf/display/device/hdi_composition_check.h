...
public:
    ...
    int32_t Check(std::vector<LayerSettings> &layers, BufferHandle &clientBuffer, uint32_t checkType = CHECK_VERTEX);
...
private:
    ...
    std::vector<uint32_t> GetCheckColors(std::vector<LayerSettings> &layers, const std::vector<Point> &points);
    HdiCompositionCheck() {}
    ~HdiCompositionCheck() {}
    uint32_t mDispW = 0;
    uint32_t mDispH = 0;
