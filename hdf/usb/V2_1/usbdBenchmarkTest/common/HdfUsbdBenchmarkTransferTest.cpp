...
(benchmark::State& st)
{
    ...
    auto ret = -1;
    for (auto _ : st) {
        ret = g_usbHostInterface->ControlTransferRead(dev, ctrlparmas, bufferData);
    }
    ASSERT_EQ(0, ret);
...
(benchmark::State& st)
{
    ...
    auto ret = -1;
    for (auto _ : st) {
        ret = g_usbHostInterface->ControlTransferWrite(dev, ctrlparmas, bufferData);
    }
    ASSERT_EQ(0, ret);
    ReleasePara(subscriber);
}
...
(benchmark::State& st)
{
    ...
    auto ret = -1;
    for (auto _ : st) {
        ret = g_usbHostInterface->BindUsbdHostSubscriber(subscriber);
    }
    ASSERT_EQ(0, ret);
    ReleasePara(subscriber);
}
...
(benchmark::State& st)
{
    ...
    for (auto _ : st) {
        ret = g_usbHostInterface->BindUsbdHostSubscriber(subscriber);
        dev_ = {subscriber->busNum_, subscriber->devAddr_};
        ret = g_usbHostInterface->UnbindUsbdHostSubscriber(subscriber);
    }
    ASSERT_EQ(0, ret);
}
...
(benchmark::State& st)
{
    ...
    for (auto _ : st) {
        std::vector<uint8_t> bufferData(MAX_BUFFER_LENGTH);
        ret = g_usbHostInterface->BulkTransferReadwithLength(
            dev, pipe, TRANSFER_TIME_OUT, bufferData.size(), bufferData);
    }
    EXPECT_EQ(0, ret);
    ReleasePara(subscriber);
}
...
(benchmark::State& st)
{
    ...
    auto ret = -1;
    for (auto _ : st) {
        ret = g_usbHostInterface->ControlTransferReadwithLength(dev, ctrlparmas, bufferData);
    }
    EXPECT_EQ(0, ret);
    ReleasePara(subscriber);
}
