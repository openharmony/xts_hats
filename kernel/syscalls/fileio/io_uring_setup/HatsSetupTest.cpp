...
HWTEST_F(HatsSetupTest, SetupApiTestSuccess_0001, Function | MediumTest | Level1)
{
    ...
    int uringFd01 = io_uring_setup(1, &p);
    EXPECT_TRUE(uringFd01 > 0);

    int uringFd02 = io_uring_setup(2, &p);
    EXPECT_TRUE(uringFd02 > 0);

    int uringFd03 = io_uring_setup(8, &p);
    EXPECT_TRUE(uringFd03 > 0);

    int uringFd04 = io_uring_setup(32767, &p);
    EXPECT_TRUE(uringFd04 > 0);

    int uringFd05 = io_uring_setup(32768, &p);
    EXPECT_TRUE(uringFd05 > 0);
}
...
HWTEST_F(HatsSetupTest, SetupApiTestFail_0003, Function | MediumTest | Level1)
{
    ...
    int uringFd01 = io_uring_setup(-1, &p);
    EXPECT_EQ(uringFd01, -1);
    EXPECT_EQ(errno, 22);

    int uringFd02 = io_uring_setup(0, &p);
    EXPECT_EQ(uringFd02, -1);
    EXPECT_EQ(errno, 22);

    int uringFd03 = io_uring_setup(32769, &p);
    EXPECT_EQ(uringFd03, -1);
    EXPECT_EQ(errno, 22);
}
...
HWTEST_F(HatsSetupTest,SetupApiTest_params_int_0004,Function | MediumTest | Level1)
{
     ...
     memset_s(&params, sizeof(int), 0, sizeof(int));
